package com.deemons.serialportlib.utils;
...
public class crc17 {
    ...
    public static int crcVal1(byte[] pcMess,int wLen) {
        long MSBInfo = 0;
        int i, j;
        int nCRCData;
        nCRCData = 0xffff;
    ...
    public static final int[] calculateCRC(byte[] data, int offset, int len) {
        ...
        public static byte[] computeCrc(CrcType crcType, byte[] data) {
            ...
            return new byte[] {  (byte) (wCrcU >> 8),(byte) wCrcU };
        }

        private static short updateCrcU(byte charU, short lpwCrcU) {
            charU = (byte) (charU ^ lpwCrcU);
            charU = (byte) (charU ^ charU << 4);
            return (short) ((lpwCrcU & 0xffff) >> 8 ^ (charU & 0xFF) << 8 ^ (charU & 0xFF) << 3 ^ (charU & 0xFF) >> 4);
        }
